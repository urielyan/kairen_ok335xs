...
void count_kb_value::on_pushButton_clicked()
{
  ...
  int count_input_s = 0;
  QVector<int> count_input_s_index;
  for(int i = 0; i < 12 ; i++){
  ...
  if(ui->comboBox->currentText().toInt() <= WORK_CURVE_2_MAX){
      ...
      printer_result(count_input_s_index);
      return;
  }
}
...
void count_kb_value::printer_result(QVector<int> count_input_s_index){
  int enter = 0x0A;

  qDebug() << count_input_s_index;
  ...
  for(int i = datalist.size() ; i >= 1  ; i--){
      ...
      //第几组数据
      QString currentInputSIndex = QString::number(count_input_s_index[i - 1] + 1);
      printer::transmit(currentInputSIndex.toLocal8Bit().data()
                        , currentInputSIndex.size());
      printer::transmit('#');
